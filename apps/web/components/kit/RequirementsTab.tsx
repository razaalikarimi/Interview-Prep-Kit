  ...
  return (
    <div className="space-y-6 max-w-4xl animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
        <div>
          <h2 className="text-base font-semibold text-white">Target Role Requirements</h2>
      ...
      <div className="enterprise-card p-6">
        <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-800">
          ...
          <span className="text-[11px] text-slate-500">Essential for interview passing score</span>
        </div>

        {filteredMust.length === 0 ? (
          <p className="text-xs text-slate-500 py-2">No must-have requirements in this category.</p>
        ) : (
          <div className="space-y-2.5">
            {filteredMust.map((req) => (
              <div
                key={req.id}
                className="flex items-start justify-between gap-3 p-3 rounded bg-slate-900 border border-slate-800 text-xs"
              >
                <div className="flex items-start gap-2.5 min-w-0">
                  <span className="font-mono text-[11px] text-slate-400 px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 mt-0.5 flex-shrink-0">
                    {req.id}
                  </span>
                  <span className="text-slate-200 font-medium leading-relaxed">{req.text}</span>
                </div>
      ...
      <div className="enterprise-card p-6">
        <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-800">
          ...
          <span className="text-[11px] text-slate-500">Differentiating qualifications</span>
        </div>

        {filteredNice.length === 0 ? (
          <p className="text-xs text-slate-500 py-2">No nice-to-have requirements in this category.</p>
        ) : (
          <div className="space-y-2.5">
            {filteredNice.map((req) => (
              ...
              >
                <div className="flex items-start gap-2.5 min-w-0">
                  <span className="font-mono text-[11px] text-slate-500 px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 mt-0.5 flex-shrink-0">
                    {req.id}
                  </span>
