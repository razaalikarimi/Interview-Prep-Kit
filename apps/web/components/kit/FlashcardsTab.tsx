    ...
    <div className="space-y-6 max-w-5xl animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base font-semibold text-white">Interview Flashcards</h2>
            <span className="text-xs text-slate-400">({flashcards.length} cards)</span>
          </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-medium rounded border border-slate-800 transition-colors"
          >
      ...
      {flashcards.length === 0 ? (
        <div className="enterprise-card p-10 text-center text-xs text-slate-400">
          <p className="mb-3 font-medium text-slate-300">No flashcards available in this kit.</p>
          <button
            onClick={() => setIsAddingCard(true)}
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-800/80">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-[10px] text-slate-500 px-1.5 py-0.2 rounded bg-slate-900 border border-slate-800">
                        {card.id}
                      </span>
                ...
                {card.requirement_ids && card.requirement_ids.length > 0 && (
                  <div className="mt-3 pt-2 border-t border-slate-800/40 flex items-center gap-1.5 flex-wrap">
                    <span className="text-[10px] text-slate-500 font-mono">Linked:</span>
                    {card.requirement_ids.map((rId) => (
                      <span
                        key={rId}
                        className="text-[10px] font-mono px-1 rounded bg-slate-900 text-slate-400 border border-slate-800"
                      >
                        {rId}
                      </span>
                    ))}
      ...
      >
        <div className="space-y-4 text-xs">
          <div>
            <label className="block font-medium text-slate-300 mb-1">Front (Prompt)</label>
            <textarea
              rows={2}
          ...
          <div>
            <label className="block font-medium text-slate-300 mb-1">Back (Explanation)</label>
            <textarea
              rows={4}
              value={editForm.back}
      ...
      >
        <p className="text-xs text-slate-300">
          Card: <span className="font-semibold text-white">"{deletingCard?.front}"</span>
        </p>
      </Modal>
        ...
        <form onSubmit={handleAddCard} className="space-y-4 text-xs">
          <div>
            <label className="block font-medium text-slate-300 mb-1">
              Front (Prompt / Question) <span className="text-red-400">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="block font-medium text-slate-300 mb-1">
              Back (Core Answer / Definition) <span className="text-red-400">*</span>
            </label>
            <textarea
              rows={3}
              required
